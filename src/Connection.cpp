...
Connection::Connection(Node& nodeIn, PortIndex portIndexIn, Node& nodeOut, PortIndex portIndexOut)
                           : _id(QUuid::createUuid())
                            , _outNode(&nodeOut)
                            , _inNode(&nodeIn)
...
Connection::~Connection() {
  ...
  if (_ahmedNode != nullptr)
    _ahmedNode->nodeDataModel()->Warning(false);
...
void Connection::setNodeToPort(Node& node, PortType portType, PortIndex portIndex) {
  ...
  if (portType == PortType::In && nodeWeak != nullptr) {
    QString nodeName = nodeWeak->nodeDataModel()->name();
     if (nodeName == "Output") {
       _ahmedNode = nodeWeak;
     }
  }
